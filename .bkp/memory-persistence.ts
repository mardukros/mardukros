...
/**
 * Class responsible for handling memory persistence operations
 * Provides advanced features such as integrity checks, backups, and efficient batch processing
 */
export class MemoryPersistence {
  ...
  /**
   * Enhanced constructor for MemoryPersistence with more configuration options
   ...
   * Enhanced to create backup directories as well
   */
...
export class MemoryPersistence {
  ...
  private INTEGRITY_CHECK_ENABLED: boolean;
  ...
  constructor(subsystemName: string, options: {
    ...
    // Configure options with defaults
    if (options.enableIntegrityCheck !== undefined) this.INTEGRITY_CHECK_ENABLED = options.enableIntegrityCheck;
    if (options.enableBackup !== undefined) this.BACKUP_ENABLED = options.enableBackup;
    if (options.maxRetries !== undefined) this.MAX_RETRIES = options.maxRetries;
    if (options.retryDelayMs !== undefined) this.RETRY_DELAY_MS = options.retryDelayMs;
    if (options.maxBatchSize !== undefined) this.MAX_BATCH_SIZE = options.maxBatchSize;
  ...
  private ensureStorageDirectory(): void {
    try {
      ...
      // Create backup directory if enabled
      if (this.BACKUP_ENABLED && !existsSync(this.backupDir)) {
        mkdirSync(this.backupDir, { recursive: true });
        logger.info(`Created backup directory: ${this.backupDir}`);
      }
  ...
  async loadItems(): Promise<Map<string | number, MemoryItem>> {
    ...
    try {
      ...
      return await this.retry(async () => {
        ...
        if (this.INTEGRITY_CHECK_ENABLED) {
  ...
  async saveItems(items: Map<string | number, MemoryItem>): Promise<void> {
    ...
    if (itemsArray.length > this.MAX_BATCH_SIZE) {
    ...
    try {
      ...
      if (this.INTEGRITY_CHECK_ENABLED && await this.fileExists(`${tempFile}.checksum`)) {
...
async loadSnapshot(timestamp: string): Promise<Map<string | number, MemoryItem> | null> {
  ...
  async saveMemoryItems(items: Map<string | number, MemoryItem>, batchSize = 1000): Promise<void> {
    ...
    this.MAX_BATCH_SIZE = batchSize; // Use the provided batch size

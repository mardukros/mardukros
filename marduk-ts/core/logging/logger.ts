...
export class Logger {
  ...
  async rotateLog(): Promise<void> {
    ...
    try {
      ...
      this.error('Log rotation failed', error as Error);
      throw error;
    }
  }
   ...
   */
  async logSystemPerformance(performanceData: object): Promise<void> {
    ...
    if (env.logging.performanceFile) {
      try {
        ...
        fs.appendFileSync(
          env.logging.performanceFile,
          JSON.stringify(enhancedData) + '\n'
        );
      } catch (error) {
        this.error('Failed to write performance data to file', error as Error);
      }
    }
   ...
   */
  async logErrorDetails(errorDetails: object): Promise<void> {
   ...
   */
  private captureErrorDetails(message: string, error?: Error, metadata: object = {}): void {
    try {
      // Create error report
      const errorReport = {
        message,

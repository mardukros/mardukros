...
// Interface for context cache item
...
export class ContextPersistence {
   ...
   */
  public async save(contextCache?: Map<string, ContextCacheItem>): Promise<void> {
   ...
   * @param contextCache Map containing context cache entries
   */
  public saveSync(contextCache?: Map<string, ContextCacheItem>): void {
    if (!this.isPersistenceEnabled || !contextCache) return;
   ...
   */
  private async cleanupSnapshots(maxSnapshots: number): Promise<void> {
    try {
      const snapshotsDir = join(this.storageDir, 'snapshots');
   ...
   * @param contextCache Map containing context cache entries
   */
  private async saveContextCache(contextCache: Map<string, ContextCacheItem>): Promise<void> {
    try {
      const filePath = join(this.storageDir, 'context-cache.json');
      const data = JSON.stringify(Array.from(contextCache.entries()), null, 2);
   ...
   */
  public async load(): Promise<Map<string, ContextCacheItem>> {
    const contextCache = new Map<string, ContextCacheItem>();
    ...
    try {
      ...
      const entries = JSON.parse(data) as [string, ContextCacheItem][];
   ...
   */
  public async loadSnapshot(timestamp: string): Promise<Map<string, ContextCacheItem> | null> {

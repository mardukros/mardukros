...
import { AiError, AiApiError } from '../errors/ai-errors.js';
import * as stringSimilarity from 'string-similarity';
import { VectorSimilarity } from './utils/vector-similarity.js';
import { WeightedLRUCache } from './utils/advanced-cache.js';
import { ContextSourceManager, DocumentContextSource } from './utils/context-sources.js';
...
import { ContextPersistence } from './utils/context-persistence.js';
import { ContextValidator } from './utils/context-validation.js';
export class AiCoordinator {
    ...
    constructor() {
        ...
        this.contextCache = new WeightedLRUCache({
     ...
     */
    async enrichContextFromMemory(query) {
        try {
    ...
    recordUserActivity(query) {
        ...
        const tags = query
            ...
            .slice(0, 5);
        this.userActivityAdapter.recordActivity(`User asked: ${query.length > 50 ? query.substring(0, 50) + '...' : query}`, 'user_query', tags);
    }
    /**
     * Logs distribution of context sources for analytics
     * @param items Context items
     */
    logContextSourceDistribution(items) {
        const sourceCounts = {};
     ...
     */
    async legacyEnrichContextFromMemory(query) {
        try {
            ...
            const memoryContext = [
                ...semanticResults.items.map((item) => this.formatMemoryItem(item, 'semantic')),
                ...declarativeResults.items.map((item) => this.formatMemoryItem(item, 'declarative')),
                ...episodicResults.items.map((item) => this.formatMemoryItem(item, 'episodic'))
            ];
    ...
    updateContextCache(key, context, relevance = 0.5, query = '') {
        const queryTerms = query.toLowerCase().split(/\s+/).filter(t => t.length > 2);
     ...
     */
    async calculateQueryContextRelevance(query, context) {
        ...
        try {
            ...
            const similarities = await Promise.all(samples.map(text => this.vectorSimilarity.calculateSimilarity(query, text)));
    ...
    async rankAndFilterContext(context, query) {
        if (!context || context.length === 0) {
    ...
    rankByStringSimilarity(context, query) {
    ...
    calculateStringRelevance(text, query) {
        try {
    ...
    calculateConfidence(response) {
        try { // Added try-catch
            ...
            if (response?.usage) {
                ...
                if (tokenRatio > 0.8)
                    confidence += 0.1;
                else if (tokenRatio < 0.2 && completionTokens > 0)
                    confidence -= 0.1; // Avoid penalty for zero completion
                // Adjust based on response length
                const contentLength = response?.content?.length ?? 0;
                if (contentLength > 500)
                    confidence += 0.1;
                else if (contentLength < 50 && contentLength > 0)
                    confidence -= 0.1; // Avoid penalty for empty content
            }
            else {
            ...
            if (totalCacheAccesses > 10) {
                ...
                if (hitRate > 0.7)
                    confidence += 0.05; // Bonus for good cache performance
            }
     ...
     */
    async restoreContextSnapshot(timestamp) {
        if (!this.persistenceEnabled) {
            throw new Error('Context persistence is disabled');
        }
     ...
     */
    async validateContextCache(applyFixes) {
        ...
        try {
            ...
            console.log(`Validation complete in ${validationResult.processedInMs.toFixed(0)}ms. Found ${issueCount} issues:`, `high=${severityCount.high}, medium=${severityCount.medium}, low=${severityCount.low}`);
     ...
     * @returns Promise resolving to validation result
     */
    async validateContextItems(contextItems, applyFixes) {
        if (!this.validationEnabled) {
            return { skipped: true, reason: 'validation_disabled' };
        }
        try {

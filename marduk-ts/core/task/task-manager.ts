import { TaskMessage, TaskPriority, TaskStatus } from './types/task-types.js';
...
interface TaskPriorityFactors {
  baseFactor: number;        // Base prioritization factor (1.0 by default)
  agingFactor: number;       // How much age influences priority (0.1 by default)
  dependencyFactor: number;  // How much dependency priorities influence this task (1.0 by default)
  urgencyFactor: number;     // How much urgency influences priority (1.5 by default)
  userFactor: number;        // How much user priority influences priority (2.0 by default)
  resourceFactor: number;    // How much resource availability influences priority (0.8 by default)
  decayRate: number;         // How fast priority decays when not executed (0.05 by default)
  contextBoost: number;      // Priority boost when task has relevant context (0.3 by default)
  failurePenalty: number;    // Priority penalty for previously failed tasks (0.2 by default)
  stalledBoost: number;      // Priority boost for tasks stalled for too long (0.4 by default)
}

/**
 * Category-specific prioritization rules
 */
interface CategoryPrioritizationRule {
  name: string;              // Category name (e.g., "io", "cpu", "ai")
  baseMultiplier: number;    // Base priority multiplier for this category
  minPriority?: number;      // Minimum priority for tasks in this category
  maxPriority?: number;      // Maximum priority for tasks in this category
  systemLoadImpact?: number; // How much system load affects this category (0-1)
  batchSize?: number;        // Preferred batch size for this category
  customFactors?: Partial<TaskPriorityFactors>; // Category-specific priority factors
  allowParallel?: boolean;   // Whether tasks in this category can run in parallel
  maxParallelTasks?: number; // Maximum number of parallel tasks if allowed
  isPreemptive?: boolean;    // Whether tasks in this category can preempt other tasks
}

export class TaskManager {
  private taskQueue: TaskMessage[] = [];
  private resourceMonitor: SystemResourceMonitor;
  private defaultPriorityFactors: TaskPriorityFactors = {
    baseFactor: 1.0,
    agingFactor: 0.1,
  ...
  private categoryRules: Map<string, CategoryPrioritizationRule> = new Map([
  ...
  private priorityFactors: TaskPriorityFactors;
  private stalledTaskThreshold: number = 5 * 60 * 1000; // 5 minutes
  private lastPriorityUpdate: number = Date.now();
  private taskHistory: Map<number, { attempts: number, lastFailure: number | null }> = new Map();
  private taskRelationships: Map<number, Set<number>> = new Map();
  private taskCategories: Map<string, Set<number>> = new Map();
   ...
   */
  computePriority(task: TaskMessage, includeContext: boolean = false): number {
   ...
   */
  updateAllPriorities(includeContext: boolean = true): void {
  ...
  categorizeTasks(): Record<string, TaskMessage[]> {
    const categories: Record<string, TaskMessage[]> = {};
    this.taskQueue.forEach(task => {
      const category = task.category || 'default';
  ...
  applyPriorityInheritance(): void {
    ...
    const taskMap = new Map<number, TaskMessage>();
      ...
      if (task.dependencies && task.dependencies.length > 0) {
        ...
        task.dependencies.forEach(depId => {
          ...
          if (depTask) {
            ...
            maxDependencyPriority = Math.max(
              maxDependencyPriority,
              depTask.priority ?? 0
            );
          }
        });
  ...
  applyPriorityDecay(): void {
    ...
    this.taskQueue.forEach(task => {
      if (task.priority && task.priority > 0) {
        ...
        if (task.isSystemCritical) return;
  ...
  getNextTask(categoryFilter?: string, options?: {
    ...
    for (const task of sorted) {
      // Skip excluded tasks
      if (excludeIds.includes(task.task_id)) continue;
      
      // Filter by category if specified
      if (categoryFilter && task.category !== categoryFilter) continue;
      
      // Skip deferred tasks unless explicitly included
      if (!includeDeferred && task.status === 'deferred') continue;
      
      // Skip low priority tasks
      if ((task.priority ?? 0) < priorityThreshold) continue;
  ...
  getTaskBatch(count: number, options?: {
    ...
    for (const task of sorted) {
      ...
      // Skip low priority tasks
      if ((task.priority ?? 0) < minPriority) continue;
   ...
   */
  private notifyTaskCompletion(taskId: number): void {
